...
def sunspot_view(request):
    ...
    if city:
        ...
        if redis_client:
            sun_data = get_cached_sunspot_by_city(city_name, resolved_date_str)
            if sun_data:
                logger.info(f"Found cached data for city: {city_name}")
                # Extract coordinates from the cache key
                pattern = f"{settings.REDIS_KEY_PREFIX}:{city_name}:*:{resolved_date_str}"
                keys = redis_client.keys(pattern)
                if keys:
                    key_parts = keys[0].split(":")
                    cached_lat = key_parts[2]  # city:lat:lon:date
    ...
    # Case 2: Query by coordinates
        ...
        try:
            ...
            if redis_client:
                sun_data = get_cached_sunspot_by_coords(lat, lon, resolved_date_str)
                if sun_data:
                    logger.info(f"Found cached data for coordinates: {lat}, {lon}")
                    # Extract city name from cache key
                    pattern = f"{settings.REDIS_KEY_PREFIX}:*:{lat}:{lon}:{resolved_date_str}"
                    keys = redis_client.keys(pattern)
                    if keys:
                        key_parts = keys[0].split(":")
